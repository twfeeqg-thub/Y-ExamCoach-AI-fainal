...
export interface ToastEventDetail {
  title?: string;
  message: SupportMessage | string;
  type?: 'success' | 'processing' | 'info' | 'warning' | 'offline' | 'error';
  duration?: number;
}
...
export const SupportToast: React.FC = () => {
  ...
  const getBorderAndBg = () => {
    switch (toast.type) {
      ...
      case 'error':
        return 'border-rose-500 bg-rose-50 dark:bg-rose-950/90 text-rose-950 dark:text-rose-100 shadow-rose-500/10';
      case 'warning':
        return 'border-amber-500 bg-amber-50 dark:bg-amber-950/90 text-amber-950 dark:text-amber-100 shadow-amber-500/10';
      default:
  ...
  const messageText = typeof toast.message === 'string' ? toast.message : toast.message?.text;
  const messageSubtext = typeof toast.message === 'string' ? undefined : toast.message?.subtext;
  const messageIcon =
